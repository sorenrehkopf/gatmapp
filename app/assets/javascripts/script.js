...
$("#userModSrchBtn").on('click',function(e){
		...
		var rslt=$.ajax({
			...
			console.log(rslt);
			for(i=0;i<rslt.responseJSON.length;i++){
			$('#usrSrchRslts').append("<div><p><img src='http://res.cloudinary.com/hx0wmncm4/image/upload/c_fill,h_40,w_40/"+rslt.responseJSON[i].pic+"'><strong>"+rslt.responseJSON[i].user_name+"</strong><button class='followBtn' userId='"+rslt.responseJSON[i].pic+"'>Follow.</button><div class='clear'></div></p></div>");
			};
    	});
	});
